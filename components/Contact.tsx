          ...
          <div className="bg-[#111] p-6 border border-gray-800 rounded hover:border-gray-700 transition-colors">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button 
                ...
                className={`w-full py-3 mt-4 rounded font-mono font-bold text-sm uppercase tracking-wider transition-all duration-300 flex items-center justify-center space-x-2 ${
                  status === 'IDLE' 
                    ? 'bg-neon-blue text-background hover:bg-white hover:shadow-[0_0_15px_rgba(0,255,255,0.5)]' 
                    : status === 'PROCESSING'
                    ? 'bg-gray-700 text-gray-300 cursor-wait'
                    : 'bg-neon-green text-background cursor-default'
                }`}
              >
                {status === 'IDLE' && (
                    <>
                ...
                {status === 'PROCESSING' && <span>[TRANSMITTING...]</span>}
                {status === 'SENT' && <span>[MESSAGE_DELIVERED]</span>}
              </button>
            </form>
          </div>
        </div>
